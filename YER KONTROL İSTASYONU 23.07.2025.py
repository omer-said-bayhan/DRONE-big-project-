...
import os
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QLabel,QMessageBox, QTabWidget, QPushButton, QFrame, QVBoxLayout, QLineEdit, QHBoxLayout, QFileDialog,QTextEdit
...
def listen_for_coordinates():
    ...
    print(f"YKI {PORT} portunda koordinat bekliyor...")

    while True:
        print("Dinliyorum...")
        try:
            data, addr = sock.recvfrom(1024)
            print("Veri geldi:", data.decode())
            decoded = data.decode()
            lat, lon, alt = map(float, decoded.split(","))
            X = (lat, lon, alt)
            print(f"Gelen koordinatlar X'e atandı: {X}")
        except Exception as e:
            print("Alım hatası:", str(e))


def listen_for_coordinates_thread():
    threading.Thread(target=listen_for_coordinates, daemon=True).start()

#com9 kartfrom PyQt5.QtWebEngineWidgets import QWebEngineSettings
#'udp:127.0.0.1:14550' ip gazebo cart curt
#-35.36321719 149.16516857 avusturalya cart curts
ipdrone = "COM14"
ipkmkz ="COM16"
...
class YerKontrolIstasyonu(QMainWindow):
    ...
    def __init__(self):
        ...
        self.kmkz=None
        global ipkmkz
        global listen_for_coordinates
        global kmkzip
        global kmkzlat
        global kmkzlon
        global PORT

        self.kmkz_lock = threading.Lock() 
        listen_for_coordinates_thread()
        self.gui_guncelle.connect(self.guiye_yaz)

        self.timergmg = QTimer()
        self.timergmg.timeout.connect(self.guncelle_mod )
        self.timergmg.start(1500) 
        self.timergmg = QTimer()
        self.timergmg.timeout.connect(self.guncelle_mod)
        self.timergmg.start(1500) 

   
        self.timergncakooordinat = QTimer()
        self.timergncakooordinat.timeout.connect(self.koordinatlari_guncelle)
        self.timergncakooordinat.start(1000)


        self.timergmk = QTimer()
        self.timergmk.timeout.connect(self.guncelle_modkmkz)
        self.timergmk.start(1500) 
        ...
        self.terminal_g_paneli.setGeometry(1070, 500, 1500, 4800)
       ...
       #bağlan butonu
        ...
        self.gorev_kontrol_alt_tabs.setGeometry(1070, 90, 850, 400)
        ...
        self.Gozlemci_paneli_tab.setGeometry(00, 00, 900, 400)
        ...
        self.button_Gozlemci_kesfebaslagzlm.move(340, 100)
        ...
        self.Kamikaze_paneli_tab.setGeometry(00, 00, 900, 400)
        ...
        #kamikaze gözlem pil
        self.button_dosyayukle = QPushButton("dosya aktar",  self.Kamikaze_paneli_tab)
        self.button_dosyayukle.setFixedSize(200, 100)
        self.button_dosyayukle.move(300, 100)
        self.button_dosyayukle.setStyleSheet("""
            background-color: rgb(73, 132, 160,0.999999); 
            border: 2px solid black;               
            border-radius: 20px;                  
            font-size: 15px;                       
            padding: 10px;                      
        """)
        self.button_dosyayukle.clicked.connect(self.dosyayukle)
        ...
        self.button_Kamikaze_RTLkmkz.move(530, 100)
        ...
        self.dusman_coord_input = QLabel(self.gorev_kontrol_tab)
        self.dusman_coord_input.setFixedSize(300, 50)
        self.dusman_coord_input.move(610, 813)
        self.dusman_coord_input.setStyleSheet("""
            background-color: rgba(73, 132, 160, 0.999999);   
            border: 2px solid black;
            border-radius: 20px;              
            font-size: 15px;           
            padding: 10px;                         
        ...
        self.button_dusman_goster = QPushButton("Düşman koordinatlarını  \nHaritada Göster", self.gorev_kontrol_tab)
        ...
        self.terminal_paneli = QWidget(self.gozlem_tab)  
        ...
        self.timer_modekmkz.start(1000)
          ...
          # Arm durumu butonunu oluştur
        self.button_Gozlemci_paneli_armdurumugzlm_gkmkz_tab = QPushButton("Arm Durumunu Değiştir", self.Kamikaze_gkmkz_tab)
        ...
        self.button_Gozlemci_paneli_aarmdurumu_gkmkz_tab.move(550, 100)
        ...
        self.timer_armkmkz.timeout.connect(self.goster_Aarmkmkzp_durumu)
        ...
        try:
            # ArduPlane ile bağlantı kur
            print("Bağlanıyor...")
            self.kmkz = connect(ipkmkz, wait_ready=False, baud=57600)
            print("Bağlantı başarılı!")
            time.sleep(5)
            # ArduPlane mod haritasını güvenli şekilde güncelle

            # Mevcut modu yazdır
            print(f"Aktif uçuş modu: {self.kmkz.mode.name}")

            # GUIDED moda geçmeye çalış
            print("GUIDED moduna geçiliyor...")
            self.kmkz.mode = VehicleMode("GUIDED")
            time.sleep(2)

            # GUIDED moda geçene kadar bekle
            timeout = 10
            while self.kmkz.mode.name != "GUIDED" and timeout > 0:
                print(f"Bekleniyor... Şu anki mod: {self.kmkz.mode.name}")
                time.sleep(1)
                timeout -= 1

            if self.kmkz.mode.name == "GUIDED":
                print("GUIDED moduna başarıyla geçildi!")
            else:
                print("GUIDED moduna geçilemedi.")

            # GPS durumu kontrolü
            print(f"GPS fix durumu: {self.kmkz.gps_0.fix_type}")
            if self.kmkz.gps_0.fix_type < 3:
                print("UYARI: 3D GPS fix yok (fix_type < 3)")

        except Exception as e:
            print(f"Bağlantı kurulamadı veya hata oluştu: {e}")
            self.terminal_output.append(f"Bağlantı kurulamadı: {e}")
            self.terminal_g_output.append(f"Bağlantı kurulamadı: {e}")
        ...
        self.dusmanlar = [] 
        self.dusmanlari_yukle()  

        self.map_center = (41.1964508, 29.2432839)  #   40.99669628175805, 29.060363489371678   40.998919188798126, 29.05346102959469         -35.36321719 ,149.16516857
    ...
    def dusmanlari_yukle(self):
        ...
        self.dusmanlar = []
        try:
            dosya_yolu = self.dusman_coord_input.text().strip()  
                ...
                for line in f:
                    ...
                    if "Düşman eklendi" in line:
                        try:
                            ...
                            kmkzlat, kmkzlon = float(lat_str.strip()), float(lon_str.strip())
                            self.dusmanlar.append((kmkzlat, kmkzlon))
                        except ValueError:
    ...
    def harita_guncelle(self):
        ...
        try:
            ...
            print(drone_konum)
            """kmkz_lat = self.kmkz.location.global_frame.lat
            kmkz_lon = self.kmkz.location.global_frame.lon
            kamikaze_konum = (kmkz_lat, kmkz_lon)"""
            ...
            folium.Marker(location=drone_konum, tooltip="Drone", icon=folium.Icon(color="green")).add_to(self.m)
            folium.Marker(location=drone_konum, tooltip="Drone", icon=folium.Icon(color="green")).add_to(self.m_g)

            """folium.Marker(kamikaze_konum, tooltip="kamikaze", icon=folium.Icon(color="black")).add_to(self.m)
            folium.Marker(kamikaze_konum, tooltip="kamikaze", icon=folium.Icon(color="black")).add_to(self.m_g)"""
            ...
            # Zigzag rota varsa ekle
            if hasattr(self, 'dusman_coord_input') and  self.dusman_coord_input:
                folium.PolyLine( self.dusman_coord_input, color="black", weight=2.5).add_to(self.m)
                folium.PolyLine( self.dusman_coord_input, color="black", weight=2.5).add_to(self.m_g)
            ...
            self.terminal_output.append(f"Konum alınamadı: {e}")
            self.terminal_g_output.append(f"Konum alınamadı: {e}")
    ...
    def ciz_kare(self):
        ...
        # Zigzag rota üretimi
        rota = []
        #metre_basina_lon = 1 / 85200         eski      




        ortalama_lat = (lat_min + lat_max) / 2
        metre_basina_lon = 1 / (111320 * cos(radians(ortalama_lat)))  # 🔧 Enleme göre düzeltildi

        adim = 5  # metre cinsinden
        current_lon = lon_min
        yukari = True
    ...
    def zigzag_waypoint_yukle(self):
        ...
        try:
            ...
            for i, (lat, lon) in enumerate(self.zigzag_rota):
                ...
                self.terminal_output.append(f"Eklenen {i}. waypoint: Enlem={lat}, Boylam={lon}")
                self.terminal_g_output.append(f"Eklenen {i}. waypoint: Enlem={lat}, Boylam={lon}")
                wp = Command(
                    0, 0, 0,
    ...
    def on_button_dusman_goster_clicked(self):
        try:
            ...
            # Koordinatları 'lat,lon' formatında ayır
            lat_lon = text.split(",")
            if len(lat_lon) != 2:
                raise ValueError("Koordinatlar 'lat,lon' formatında olmalı.")
            dusman_coord = tuple(map(float, lat_lon))  # Koordinatları float olarak al
            # Düşmanı listeye ekle
            self.dusmanlar.append(dusman_coord)
            # Haritayı güncelle
            self.harita_guncelle()
        except ValueError:
            print("Geçersiz koordinatlar. Lütfen 'Lat, Long' formatında girin.")
            self.terminal_output.append("Geçersiz koordinatlar. Lütfen 'Lat, Long' formatında girin.")
            self.terminal_g_output.append("Geçersiz koordinatlar. Lütfen 'Lat, Long' formatında girin.")
        except Exception as e:
            print(f"Bir hata oluştu: {e}")
    ...
    def on_button_Gozlemci_RTLgzlm_clicked(self):
        try:
            ...
            time.sleep(1)  # Mod değişimini beklemek için kısa gecikm
    ...
    def on_button_Kamikaze_RTLkmkz_clicked(self):
      self.kmkz.mode=VehicleMode("LAND")
    ...
    def on_button_Gozlemci_paneli_armdurumugzlm_g_clicked(self):
        try:
            ...
            print(f"Beklenmedik bir hata oluştu: {e}")
            self.terminal_output.append(f"Beklenmedik bir hata oluştu: {e}")
            self.terminal_g_output.append(f"Beklenmedik bir hata oluştu: {e}")
    ...
    def goster_Aarmkmkzp_durumu(self):
        try:
            ...
            if not kmkzip.armed:
                ...
                # Arm işlemi tamamlanana kadar bekle
                while not kmkzip.armed:
                    time.sleep()

                # Bir kere yaz, döngü dışında
    ...
    def on_button_baglan_clicked(self):
        try:
            self.vehicle = connect(ipdrone ,wait_ready=False, baud=57600)
    ...
    def arm_veya_disarm(self):
        try:
            with self.kmkz_lock:  # Thread-safe erişim
                kmkzip = self.kmkz

                if kmkzip is None:
                    print("Kamikaze aracı bağlı değil.")
                    self.terminal_output.append("Kamikaze aracı bağlı değil.")
                    self.terminal_g_output.append("Kamikaze aracı bağlı değil.")
                    return
                if kmkzip.armed:
                    print("Kamikaze disarm ediliyor...")
                    self.terminal_output.append("Kamikaze disarm ediliyor...")
                    self.terminal_g_output.append("Kamikaze disarm ediliyor...")

                    kmkzip.armed = False
                    while kmkzip.armed:
                        print("Disarm için bekleniyor...")
                        time.sleep(1)
                    print("Kamikaze disarm oldu.")
                    self.terminal_output.append("Kamikaze disarm oldu.")
                    self.terminal_g_output.append("Kamikaze disarm oldu.")
                else:
                    print("Kamikaze arm ediliyor...")
                    self.terminal_output.append("Kamikaze arm ediliyor...")
                    self.terminal_g_output.append("Kamikaze arm ediliyor...")
                    kmkzip.mode = VehicleMode("GUIDED")
                    while kmkzip.mode.name != "GUIDED":
                        print("GUIDED moda geçiliyor...")
                        self.terminal_output.append("GUIDED moda geçiliyor...")
                        self.terminal_g_output.append("GUIDED moda geçiliyor...")
                        time.sleep(1)
                    kmkzip.armed = True
                    while not kmkzip.armed:
                        print("Arm için bekleniyor...")
                        time.sleep(1)
                    print("Kamikaze arm oldu.")
                    self.terminal_output.append("Kamikaze arm oldu.")
                    self.terminal_g_output.append("Kamikaze arm oldu.")
        except Exception as e:
            hata = f"HATA (kamikaze arm/disarm): {e}"
            self.terminal_output.append(hata)
            self.terminal_g_output.append(hata)
            
    def kamikaze_arm_disarm_thread(self):
        threading.Thread(target=self.arm_veya_disarm, daemon=True).start()
    ...
    def gorev_takibi(self):
        try:
            toplam_wp = self.vehicle.commands.count
            ...
            # Son waypoint hedeflendiğine göre, görev tamamlanmış sayılabilir
            print("Son waypoint'e ulaşıldı, görev tamamlanıyor...")
            self.terminal_output.append("Son waypoint'e ulaşıldı, görev tamamlanıyor...")
            self.terminal_g_output.append("Son waypoint'e ulaşıldı, görev tamamlanıyor...")

            time.sleep(5)  # Son WP'ye ulaşma ve durma süresi için bekle

            print("Görev tamamlandı, RTL moduna geçiliyor...")
            self.vehicle.mode = VehicleMode("RTL")
            time.sleep(2)  # Modun oturması için bekle

            if self.vehicle.mode.name == "RTL":
                print(" RTL moduna başarıyla geçildi.")
            else:
                print(" RTL moduna geçiş başarısız.")

            self.gui_guncelle.emit("Görev tamamlandı. Tüm waypoint'ler uçuldu.")
    ...
    def on_button_Kamikaze_kesfebaslakmkz_clicked(self):
            ...
            flight_thread = threading.Thread(target=self.start_flight)
            flight_thread.start()
    ...
    def start_flight(self):
        kmkzip = self.kmkz
        """current_location = kmkzip.location.global_frame
        heading = kmkzip.heading

        # Trigonometrik düzleme uygun hale getir
        heading_rad = math.radians(heading )

        # İlk waypoint: 150 metre ileri
        dNorth = 150 * math.cos(heading_rad)
        dEast = 150 * math.sin(heading_rad)
        new_wp = get_location_metres(current_location, dNorth, dEast)

        # 90 derece sağa 50 metre
        heading_rad_sag = math.radians(heading-90 )
        dNorth2 = 50 * math.cos(heading_rad_sag)
        dEast2 = 50 * math.sin(heading_rad_sag)
        new_wp2 = get_location_metres(new_wp, dNorth2, dEast2)

        # Geri 150 metre
        heading_rad_geri = math.radians(heading - 180)
        dNorth3 = 150 * math.cos(heading_rad_geri)
        dEast3 = 150 * math.sin(heading_rad_geri)
        new_wp3 = get_location_metres(new_wp2, dNorth3, dEast3)

        # 50 metre sola
        heading_rad_sola = math.radians(heading-270)
        dNorth4 = 50 * math.cos(heading_rad_sola)
        dEast4 = 50 * math.sin(heading_rad_sola)
        new_wp4 = get_location_metres(new_wp3, dNorth4, dEast4)

        # Dalış noktası sabit
        dalma_lat = 40.99581032398173
        dalma_lon = 29.059603967084257
        dalis_irtifa = 60

        # Dalış öncesi: 5 metre geriden
        heading_rad_dalis = math.radians(heading )
        dNorth_oncesi = -5 * math.cos(heading_rad_dalis)
        dEast_oncesi = -5 * math.sin(heading_rad_dalis)
        dalma_baslangic = get_location_metres(
            LocationGlobal(dalma_lat, dalma_lon, dalis_irtifa),
            dNorth_oncesi, dEast_oncesi
        )

        # Dalış çıkışı: 30 metre ileri (dalış başından itibaren)
        dNorth_sonrasi = 50 * math.cos(heading)
        dEast_sonrasi = 50 * math.sin(heading)
        dalma_cikis_nokta = get_location_metres(
            dalma_baslangic,
            dNorth_sonrasi ,  
            dEast_sonrasi 
        )

        home_location = kmkzip.location.global_frame
        cmds = kmkzip.commands
        cmds.clear()
        cmds.wait_ready()
        ...
        cmds.add(Command(0, 0, 0,
            ...
            0, 0, 0, 0, 0, 0,
            new_wp.lat, new_wp.lon, 10))

        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp.lat, new_wp.lon, 40))

        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp2.lat, new_wp2.lon, 50))

        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp3.lat, new_wp3.lon, 60))

        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp4.lat, new_wp4.lon, 60))
        ...
        cmds.add(Command(0, 0, 0,
            ...
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
             dalma_lat, dalma_lon, 60))


        # Script komutu (18: özel bir script komutuysa bu şekilde kalmalı)
        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
        ...
        cmds.add(Command(0, 0, 0,
            ...
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp2.lat, new_wp2.lon, 30))

        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp3.lat, new_wp3.lon, 20))

        cmds.add(Command(0, 0, 0,
            mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
            new_wp4.lat, new_wp4.lon, 10))
        ...
        # Görev yükle ve indir
        cmds.upload()
        print("Görev rotası yükleniyor...")
        cmds.download()
        cmds.wait_ready()

        print(f"Yüklenen görev sayısı: {len(cmds)}")
        if len(cmds) == 0:
            print("⚠️ Uyarı: Hiç görev yüklenmedi!")
        else:
            print("✅ Görev başarıyla yüklendi.")

        # Uçuş modlarını ayarla
        kmkzip.mode = VehicleMode("TAKEOFF")
        while kmkzip.mode.name != "TAKEOFF":
            print("TAKEOFF moduna geçiliyor...")
            time.sleep(1)
        time.sleep(5)

        kmkzip.mode = VehicleMode("AUTO")
        while kmkzip.mode.name != "AUTO":
    ...
    def koordinatlari_guncelle(self):
        try:
            global X
            if X:
                lat, lon = X[:2]  # Sadece ilk iki elemanı al
                lat, lon, alt = X
                self.dusman_coord_input.setText(f" {lat:.6f}, {lon:.6f}")
            else:
                self.dusman_coord_input.setText("Koordinat bekleniyor...")
        except Exception as e:
            print("Hata koordinatlari_guncelle içinde:", e)
            self.terminal_output.append("Hata koordinatlari_guncelle içinde:", e)
            self.terminal_g_output.append("Hata koordinatlari_guncelle içinde:", e)
    ...
    def dosyayukle(self):
            ...
            file_name, _ = QFileDialog.getOpenFileName(self, "Waypoint Dosyasını Seçin", "", "Waypoint Files (*.waypoints);;All Files (*)", options=options)
            if file_name:
                self.dosyayazdir(file_name)


    def dosyayazdir(self, file_path):
        try:
            cmds = self.vehicle.commands
            cmds.clear()
            ...
            with open(file_path, 'r') as file:
                for line in file.readlines():
                    ...
                    cmd = Command(0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
                                  int(line_data[3]), 0, 0, float(line_data[4]), float(line_data[5]), float(line_data[6]),
                                  float(line_data[7]), float(line_data[8]), float(line_data[9]), float(line_data[10]))
                    cmds.add(cmd)
            
            cmds.upload()
            self.kmkz.mode=VehicleMode("AUTO")
            print("Waypoints başarıyla yüklendi!")

        except Exception as e:
            print(f"Waypoint yükleme hatası: {str(e)}")
            self.terminal_output.append(f"Waypoint yükleme hatası: {str(e)}")
            self.terminal_g_output.append(f"Waypoint yükleme hatası: {str(e)}")
