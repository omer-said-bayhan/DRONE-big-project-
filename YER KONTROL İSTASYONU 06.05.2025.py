...
class YerKontrolIstasyonu(QMainWindow):
    def __init__(self):
        ...
        self.timeraarmkmkz.timeout.connect(self.goster_Aarmkmkz_durumu)
       ...
       #bağlan butonu
        ...
        self.label_battery.move(550, 300)
        ...
        self.button_Gozlemci_paneli_disarmdurumugzlm_g_tab = QPushButton("Disrm Et", self.gozlemci_paneli_g_tab)
        self.button_Gozlemci_paneli_disarmdurumugzlm_g_tab.setFixedSize(200, 100)
        self.button_Gozlemci_paneli_disarmdurumugzlm_g_tab.move(300, 100)
        self.button_Gozlemci_paneli_disarmdurumugzlm_g_tab.setStyleSheet("""
            background-color: rgb(73, 132, 160,0.999999);   /* Arka plan rengi */
            border: 2px solid black;               /* Kenarlık */
            border-radius: 20px;                   /* Köşe yuvarlatma */
            font-size: 15px;                       /* Yazı tipi boyutu */
        ...
        self.button_Gozlemci_paneli_disarmdurumugzlm_g_tab.clicked.connect(self.drone_disarm_et)

        # Arm durumu butonunu oluştur
        self.button_Gozlemci_paneli_armdurumugzlm_g_tab = QPushButton("Arm Et", self.gozlemci_paneli_g_tab)
    ...
    def dusmanlari_yukle(self):
        ...
        self.dusmanlar = []  # Listeyi her seferde sıfırla
        try:
            print(f"Dosya yolu: {self.dusman_coord_input}")  # Dosya yolunu yazdır
            with open(self.dusman_coord_input, "r") as f:
                for line in f:
                    if "Düşman eklendi" in line:
                        coords = line.strip().replace("Düşman eklendi: ", "").strip()

                        # Eğer koordinatlar parantezliyse (39.1, 32.1), parantezleri temizle
    ...
    def gorev_takibi(self):
        try:
            ...
            while self.vehicle.commands.next < toplam_wp:
                aktif_wp = self.vehicle.commands.next
                time.sleep(2)

            tamamlandi = "Görev tamamlandı. Tüm waypoint'ler uçuldu."
            print(tamamlandi)
            self.terminal_output.append(tamamlandi)
            self.terminal_g_output.append(tamamlandi)
            self.vehicle.mode = VehicleMode("RTL")
        except Exception as e:
            hata = f"HATA (görev takibi): {e}"
            print(hata)
            self.terminal_output.append(hata)
            self.terminal_g_output.append(hata)"""
    ...
    def arm_durumunukmkz_degistir(self):
        try:
            # Bağlantıyı kur
            self.kmkz = connect(ipkmkz, wait_ready=True)  # Bağlantıyı kuruyoruz
    ...
    def drone_disarm_et(self):
        try:
            ...
            if vehicle.armed:
                ...
                mesaj = "Drone zaten disarm durumda."
                print(mesaj)
                self.terminal_output.append(mesaj)
                self.terminal_g_output.append(mesaj)

        except Exception as e:
            hata = f"HATA (DISARM): {e}"
            print(hata)
            self.terminal_output.append(hata)
            self.terminal_g_output.append(hata)
    ...
    def drone_disarm_et_thread(self):
        threading.Thread(target=self.drone_disarm_et).start()
