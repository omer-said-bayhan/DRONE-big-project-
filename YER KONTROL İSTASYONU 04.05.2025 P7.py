...
class YerKontrolIstasyonu(QMainWindow):
    ...
    def on_button_Gozlemci_kesfebaslagzlm_clicked(self):
        try:
            ...
            if self.vehicle.mode.name != "AUTO":
                uyari = "Uyarı: AUTO moduna geçiş başarısız!"
                print(uyari)
                self.terminal_output.append(uyari)
                self.terminal_g_output.append(uyari)
            else:
                ...
                # (İsteğe bağlı) Görevi izlemek
                toplam_wp = len(self.zigzag_rota)
                while self.vehicle.commands.next < toplam_wp:
                    aktif_wp = self.vehicle.commands.next
                    durum = f"Şu an {aktif_wp + 1}. waypoint'e gidiliyor..."
                    print(durum)
                    self.terminal_output.append(durum)
                    self.terminal_g_output.append(durum)
                    time.sleep(2)
    ...
    def arm_durumunu_degistir(self):
        vehicle = connect(ipdrone, wait_ready=True) 
