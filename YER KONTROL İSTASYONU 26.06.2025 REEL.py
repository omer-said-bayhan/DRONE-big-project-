...
import threading
import math
from gazeboucgen import get_saved_coordinates
import socket

# Koordinatları almak için
...
class YerKontrolIstasyonu(QMainWindow):
    ...
    def __init__(self):
        ...
        global SAVED_COORDS
        self.kmkz_lock = threading.Lock()  # kmkz erişimi için kilit
        super().__init__()
        
        PORT = 12345
        X = None  # Global koordinat verisi burada tutulacak
       ...
       #bağlan butonu
        ...
        # Kullanım:
# Daha önceden kaydedilen koordinatlar varsa yazdır
        ...
        def listen_for_coordinates():
            ...
            sock.bind(("", PORT))  # Her IP'den geleni dinle
            ...
            while True:
                print("Dinliyorum...")
                try:
                    data, addr = sock.recvfrom(1024)  # 1 KB yeter
        ...
        # X'i başka yerlerde kullanmak için örnek fonksiyon
        def kullan_bunu():
            global X
            if X is not None:
                print(f"X içeriği: lat={X[0]}, lon={X[1]}, alt={X[2]}")
            else:
                print("Henüz veri gelmedi.")
        ...
        self.dusman_coord_input= X
        # QLabel ile koordinatları GUI'de göster
        ...
        self.gorev_verme_g_alt_tabs.addTab(self.kamera_g_tab,"kamera")
        ...
        self.kamera_g_tab.setStyleSheet("background-color: rgb(70, 87, 101); border: 2px solid black;")
        ...
        self.Gozlemci_paneli_g_tab = QFrame(self.kamera_g_tab)  
        self.Gozlemci_paneli_g_tab.setGeometry(0, 0, 800, 700)
        ...
        """def detect_faces(frame):
            Yüz tespiti örneğ
            # OpenCV'nin hazır cascade classifier'ını kullan
        ...
        def detect_edges(frame):
            Kenar tespiti örneği
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            edges = cv2.Canny(gray, 50, 150)
            return cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)

        # Socket client oluştur
        client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

        try:
            # Sunucuya bağlan
            client_socket.connect(('localhost', 8888))
            print(" Sunucuya bağlandı - Görüntü alımı başlıyor...")
            ...
            # İşlem modu seçimi
            processing_mode = "original"  # "original", "faces", "edges"
            ...
            while True:
                ...
                # Görüntüyü deserialize et
                frame = pickle.loads(frame_data)
                frame_count += 1
                
                # İşlem moduna göre frame'i işle
                if processing_mode == "faces":
                    processed_frame, face_count = detect_faces(frame.copy())
                    cv2.putText(processed_frame, f'Faces: {face_count}', (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.imshow('Alınan Görüntü - Yüz Tespiti', processed_frame)
                    
                elif processing_mode == "edges":
                    edges_frame = detect_edges(frame)
                    cv2.imshow('Alınan Görüntü - Kenar Tespiti', edges_frame)
                    
                else:
                    # Orijinal görüntüyü göster
                    cv2.putText(frame, f'Frame: {frame_count}', (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.imshow('Alınan Görüntü - Orijinal', frame)
                
                # Klavye kontrolleri
                key = cv2.waitKey(1) & 0xFF
                if key == 27:  # ESC - Çık
                    break
                elif key == ord('f'):  # F - Yüz tespiti
                    processing_mode = "faces"
                    print(" Yüz tespiti modu aktif")
                elif key == ord('e'):  # E - Kenar tespiti
                    processing_mode = "edges"
                    print(" Kenar tespiti modu aktif")
                elif key == ord('o'):  # O - Orijinal
                    processing_mode = "original"
                    print(" Orijinal görüntü modu aktif")
                elif key == ord('s'):  # S - Görüntü kaydet
        ...
        print("\nKontroller:")
        print("F - Yüz tespiti modu")
        print("E - Kenar tespiti modu") 
        print("O - Orijinal görüntü modu")
        print("S - Görüntü kaydet")
        print("ESC - Çıkış")"""
   ...
   # Arm durumu butonunu oluştur
    ...
    def harita_guncelle(self):
        ...
        try:
            ...
            # Haritayı her srosrun gazebo_ros gazebo /home/kitt/ardupilot_gazebo/worlds/iris_arducopter_runway_dk.worldeferinde sıfırdan oluştur
    ...
    def on_button_dusman_goster_clicked(self):
        ...
        try:
            # Kullanıcıdan gelen koordinatları al
    ...
    def on_button_Kamikaze_kesfebaslakmkz_clicked(self):
            self.kmkz = connect(ipkmkz, wait_ready=True)
            """Bu fonksiyonu thread içinde çalıştıracağız"""
            flight_thread = threading.Thread(target=self.start_flight)
            flight_thread.start()

    def start_flight(self):

        if not self.dusmanlar:
            dusmanyok="Hiç düşman koordinatı yüklenmemiş."
            print(dusmanyok)
            self.terminal_output.append(dusmanyok)
            self.terminal_g_output.append(dusmanyok)
            return

        # Örnek olarak ilk düşmana git
        kmkzlat, kmkzlon = self.dusmanlar[0]
        takeoff_alt = 20  # Örnek kalkış irtifası
        print(f"Uçuş başlıyor: {kmkzlat}, {kmkzlon}, {takeoff_alt}")


        # Burada uçuş başlatma komutları
        kmkzip=self.kmkz
        takeoff_alt = 50
        ...
        kmkzcikis_lat = kmkzlat
        kmkzcikis_lon = kmkzlon + kmkzcikis_lon
        ...
        cmds.add(Command(0, 0, 0,
            ...
            mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            0, 1, 0, 0, 0, 0,
             kmkzcikis_lat, kmkzcikis_lon, takeoff_alt
        ))
        ...
        kmkzip.mode = VehicleMode("STABILIZE")
        while kmkzip.mode.name != "STABILIZE":
        ...
        kmkzip.channels.overrides['3'] = 1800   
        time.sleep(5)
        kmkzip.channels.overrides['3'] = None
        ...
        try:
            while True:
                alt = kmkzip.location.global_relative_frame.alt
                self.kmkz = kmkzip  # Güncel aracı harita için tanımla
                print(f"Yükseklik: {alt:.2f} m, Konum: {kmkzip.location.global_frame.lat}, {kmkzip.location.global_frame.lon}")
                
                #self.harita_guncelle()  # Konum değiştikçe haritayı güncelle
