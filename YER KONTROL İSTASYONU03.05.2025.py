...
class YerKontrolIstasyonu(QMainWindow):
    def __init__(self):
       ...
       #bağlan butonu
        ...
        # düşman konumu
        self.dusman_coord_label = QLabel("Düşman Koordinatları:", self.gorev_kontrol_tab)
        self.dusman_coord_label.setStyleSheet("font-weight: bold; font-size: 16px;")
        self.dusman_coord_label.move(800, 970)  # Başlığın konumunu ayarlayalım
        ...
        self.button_Gozlemci_paneli_armdurumugzlm_g_tab = QPushButton("Arm Durumu: Bilinmiyor", self.gozlemci_paneli_g_tab)
    ...
    def zigzag_waypoint_yukle(self):
        print("Zigzag yükleme fonksiyonu başlatıldı.")

        # Zigzag rotası kontrolü
        if not hasattr(self, "zigzag_rota") or not self.zigzag_rota:
            print("Zigzag rotası oluşturulmamış veya boş!")
            return

        try:
            # Uçuş kontrol cihazına bağlantı kuruluyor (wait_ready kaldırıldı)
            self.vehicle = connect('com9', baund=9600,wait_ready=False)
            ...
            # Waypoint'leri ekle
            print(f"{len(self.zigzag_rota)} waypoint ekleniyor...")
            for i, (lat, lon) in enumerate(self.zigzag_rota):
                print(f"{i+1}. Nokta: Enlem={lat}, Boylam={lon}")
                wp = Command(
                    0, 0, 0,
            ...
            # Görevleri drone'a gönder
            cmds.upload()
            time.sleep(2)

            print(f"Toplam yüklenen waypoint sayısı: {cmds.count}")
            if cmds.count == len(self.zigzag_rota):
                print("Tüm waypoint'ler başarıyla yüklendi.")
            else:
                print("Bazı waypoint'ler eksik yüklenmiş olabilir.")
    ...
    def on_button_Gozlemci_paneli_armdurumugzlm_g_clicked(self):
        try:
            armed_durum = self.vehicle.armed  
            armed_message = f"Drone arm durumu: {'Hazır' if armed_durum else 'Kapalı'}"  
            print(armed_message)  
            self.terminal_output.append(armed_message)  
            self.terminal_g_output.append(armed_message) 
...
if __name__ == "__main__":
    ...
    pencere.zigzag_waypoint_yukle()  # Rotayı yükle
