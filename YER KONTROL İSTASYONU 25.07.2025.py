...
class YerKontrolIstasyonu(QMainWindow):
    ...
    def __init__(self):
       ...
       #bağlan butonu
        ...
        self.map_center = (40.99876211357406, 29.053388070822812)  #   40.99669628175805, 29.060363489371678            -35.36321719 ,149.16516857
        ...
        try:
            ...
            if hasattr(self, 'zigzag_rota') and self.zigzag_rota:
                ...
                folium.PolyLine(self.zigzag_rota, color="orange", weight=2.5).add_to(self.m_g)
                            # ROTAYI HARİTADA GÖSTER
            if hasattr(self, 'rota_koordinatlari') and self.rota_koordinatlari :
                folium.PolyLine(self.rota_koordinatlari, color="black", weight=4).add_to(self.m)
                folium.PolyLine(self.rota_koordinatlari, color="black", weight=4).add_to(self.m_g)
                #self.kmkz_rota = self.rota_koordinatlari  # opsiyonel: başka yerde tekrar kullanmak istersen
    ...
    def arm_durumunukmkz_degistir(self):
        try:
            ...
            if not kmkz.armed:
                ...
                while not kmkz.is_armable:
                    print("Kamikaze ARM için gerekli şartlar sağlanmadı.")
                    time.sleep(2)

                kmkz.mode = VehicleMode("GUIDED")
                kmkz.armed = True

                while not kmkz.armed:
                    print("Kamikaze ARM bekleniyor...")
                    time.sleep(1)
                ...
                while kmkz.armed:
                    print("Kamikaze DISARM bekleniyor...")
                    time.sleep(1)
    ...
    def goster_Aarmkmkzp_durumu(self):
        ...
        # Önceki yazı ile aynıysa tekrar güncelleme
    ...
    def Aarm_durumunu_degistirkmkz_thread(self):
    ...
    def gorev_takibi(self):
        try:
            ...
            self.terminal_output.append("Görev tamamlandı, RTL moduna geçiliyor...")
            self.terminal_g_output.append("Görev tamamlandı, RTL moduna geçiliyor...")
            ...
            self.terminal_output.append("Görev tamamlandı. Tüm waypoint'ler uçuldu.")
            self.terminal_g_output.append("Görev tamamlandı. Tüm waypoint'ler uçuldu.")
        except Exception as e:
    ...
    def dosyayazdir(self, file_path):
        try:
            cmds = self.kmkz.commands
            cmds.clear()
            
            self.rota_koordinatlari = []  # ROTAYI GÖSTERMEK İÇİN KOORDİNAT LİSTESİ
            ...
            with open(file_path, 'r') as file:
                for line in file.readlines():
                    ...
                    # Harita için koordinatları listeye ekle
                    lat = float(line_data[8])
                    lon = float(line_data[9])
                    self.rota_koordinatlari.append((lat, lon))

            cmds.upload()
            print("Rota başarılı bir şekilde atarılmıştır")
            self.terminal_output.append("Rota başarılı bir şekilde atarılmıştır")
            self.terminal_g_output.append("Rota başarılı bir şekilde atarılmıştır")
