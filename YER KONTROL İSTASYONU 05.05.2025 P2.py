...
class YerKontrolIstasyonu(QMainWindow):
    ...
    def gorev_takibi(self):
        try:
            ...
            # Mode geçişini doğrula
            for _ in range(10):
                if self.vehicle.mode.name == "RTL":
                    print("Başarıyla RTL moduna geçildi.")
                    break
                print(f"Mod değişmedi, mevcut mod: {self.vehicle.mode.name}")
                time.sleep(1)
            else:
                print("RTL moduna geçilemedi! Lütfen GCS üzerinden kontrol edin.")
